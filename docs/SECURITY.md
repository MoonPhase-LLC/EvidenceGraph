...
- **B3 — Process/network**: frontend ↔ local analysis service ↔ model runtime, all intended to
  stay on localhost, and now additionally authenticated (see T-09, `DECISIONS.md` D-009) rather
  than relying on network binding alone.
...
|---|---|---|---|---|
| T-01 | Malicious PDF exploits a parser vulnerability (memory corruption, XXE-equivalent, etc.) | B1 | Use well-maintained parsing libraries kept up to date; run parsing with strict size/time limits; a process-level containment boundary (e.g. a size/time-limited subprocess per parse) is **decided and implemented in Sprint 4** (`DECISIONS.md` D-010), not deferred to hardening — Sprint 13 verifies/hardens it adversarially rather than introducing it. Exact mechanism is still open (see `OPEN_QUESTIONS.md` S-1). | Boundary required by Sprint 4; hardening in Sprint 13 |
...
| T-09 | Unauthorized access to the local API from another local process (threat A2) or the network (threat A3) | B3 | Bind the FastAPI service strictly to `127.0.0.1`; do not expose on `0.0.0.0` or any LAN-reachable interface (mitigates A3). Localhost binding alone does **not** mitigate A2 — any other local process/user on the same machine can still reach a `127.0.0.1` port. V0.1 therefore requires a per-launch shared-secret token, generated by the Tauri host and passed to both the FastAPI child process (via environment variable, never a CLI arg) and the frontend (via Tauri IPC), required on every request (`DECISIONS.md` D-009). | Mitigated for A3 via binding discipline; mitigated for A2 via the shared-secret token requirement |
